...
vector<int> matchingStrings(vector<string> stringList, vector<string> queries) {
    ...
    int queryCounter = 0;
    for_each(queries.begin(), queries.end(), [stringList, &queryCounter, &result](string query){
        for_each(stringList.begin(), stringList.end(), [query, &queryCounter,&result](string stringElement){
            if(query == stringElement){
                result[queryCounter]++;
            }
        });
        std::cout << result[queryCounter];
        queryCounter += 1;
    });
    return result;
}
